# GRUML
Tired of collaborating over UML diagrams or don't use them at all? We feel you. We know how tedious it is to use or even think of UML diagrams for large codebases. Thus, we bring you RUML - Rectangular UML diagrams. You can know what all classes are dependent on one another, who inherits from whom, what is the flow of control of your codebase when executed in different scenarios.

# Installation
As of now, we have not released a Python package but you can use this by simply cloning it and running the command:

`python3 generate_ruml.py <path of the source codebase> <path of the source code driver> <name of the source code driver for references>`

# Work in Progress
We are [currently working with Python.org] (https://github.com/python/cpython/pull/16466#issuecomment-583693647) to make changes in pyclbr module for our GRUML tool to work. You can get started by editing pyclbr.py file as follows:
```
# /usr/local/Cellar/python/3.7.4_1/Frameworks/Python.framework/Versions/3.7/lib/python3.7/pyclbr.py
...
def readmodule(module, path=None):
    ...
    """
    print('{} is the module received.'.format(module))
    res = {}
...
def _readmodule(module, path, inpackage=None):
    ...
    print('Inside _readmodule, module={}, path={}'.format(module, path))
    ...
    try:
        for tokentype, token, start, _end, _line in g:
            if tokentype == DEDENT:
                ...
                lineno, thisindent = start
                # end_for_avi = _end
                # print('This was a class and we got the end: {}'.format(end_for_avi))
                # Close previous nested classes and defs.
                while stack and stack[-1][1] >= thisindent:
                    del stack[-1]
